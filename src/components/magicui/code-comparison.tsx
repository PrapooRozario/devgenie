...
export function CodeComparison({
    ...
    if (highlighted) {
      return (
        <div
          ...
          className={cn(
            "h-full w-full overflow-auto bg-black font-mono text-xs",
            ...
            "group-hover/left:[&>pre>code>:not(.focused)]:!opacity-70 group-hover/left:[&>pre>code>:not(.focused)]:!blur-[0.5px]",
            "group-hover/right:[&>pre>code>:not(.focused)]:!opacity-70 group-hover/right:[&>pre>code>:not(.focused)]:!blur-[0.5px]",
            "[&>pre>code>.add]:bg-[rgba(74,222,128,0.15)] [&>pre>code>.remove]:bg-[rgba(248,113,113,0.15)]",
      ...
      return (
        <pre className="h-full overflow-auto break-all bg-black p-4 font-mono text-xs text-white">
          {code}
        </pre>
      );
    }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      <div className="group relative w-full overflow-hidden rounded-lg border border-black bg-black shadow-xl">
        <div className="relative grid md:grid-cols-2">
          <div
            className={cn(
              "leftside group/left border-black md:border-r",
              hasLeftFocus &&
                "[&>div>pre>code>:not(.focused)]:!opacity-50 [&>div>pre>code>:not(.focused)]:!blur-[0.5px]",
              "[&>div>pre>code>:not(.focused)]:transition-all [&>div>pre>code>:not(.focused)]:duration-300",
            )}
          >
            <div className="flex items-center border-b border-black bg-black p-2 text-sm text-white">
              <FileIcon className="mr-2 h-4 w-4 text-white" />
              <span className="text-white">{filename}</span>
              <span className="ml-auto hidden text-white md:block">Before</span>
            </div>
            {renderCode(beforeCode, highlightedBefore)}
          </div>
          <div
            className={cn(
              "rightside group/right border-t border-black md:border-t-0",
              hasRightFocus &&
                "[&>div>pre>code>:not(.focused)]:!opacity-50 [&>div>pre>code>:not(.focused)]:!blur-[0.5px]",
              "[&>div>pre>code>:not(.focused)]:transition-all [&>div>pre>code>:not(.focused)]:duration-300",
            )}
          >
            <div className="flex items-center border-b border-black bg-black p-2 text-sm text-white">
              <FileIcon className="mr-2 h-4 w-4 text-white" />
              <span className="text-white">{filename}</span>
              <span className="ml-auto hidden text-white md:block">After</span>
            </div>
            {renderCode(afterCode, highlightedAfter)}
          </div>
        </div>
        <div className="absolute left-1/2 top-1/2 hidden h-8 w-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-md border border-black bg-black text-xs text-white shadow-md md:flex">
          VS
        </div>
      </div>
    </div>
